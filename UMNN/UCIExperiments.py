...
def train_uci(dataset, load=None, test=False, save=None, nb_steps=50, solver="CC", hidden_embeding=[300, 300, 300, 300],
    ...
    save_name = "ExperimentsResults/" + dataset + "/" if save is None else save
    ...
    for epoch in range(cur_epoch, cur_epoch + nb_epoch):
        ...
        with torch.no_grad():
            ...
            for cur_x in batch_iter(data.val.x, shuffle=True, batch_size=batch_size):
                ll_tmp, _ = model.compute_ll(cur_x.view(-1, nb_in).to(device))
                i += 1
                ll_val -= ll_tmp.mean()
            ll_val /= i
...
parser.add_argument("-nb_flow", type=int, default=1, help="Number of nets in the flow")
...
parser.add_argument("-Optim", choices=["adamBNAF", "sgd", "adam"], type=str, default="sgd", help="Optimizer")
